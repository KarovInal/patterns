...
/******/ 	var hotCurrentHash = "deae2da62cf478e971be"; // eslint-disable-line no-unused-vars
...
class Base {
  ...
  reseive(type, id) {
    switch(type) {
      case 'analyze':
        if(id === '2') {
          console.log('display comment field...');
        }
      default:
        return null;
    }
...
class Selector {
  ...
  onSelected({ target }) {
    this.mediator.publish('selected', 'analyze', target.value);
  }

  reseive(type, ...args) {
    console.log('Get args!', type);
  }
};

let mediator = new Mediator;
let selector = new Selector(mediator);
let base = new __WEBPACK_IMPORTED_MODULE_0__Base_js__["a" /* default */](mediator);
